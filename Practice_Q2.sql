...
CREATE TABLE KHOA (
	MAKHOA varchar(4) PRIMARY KEY,--Mã khoa (khóa chính) 
	TENKHOA varchar(40),--Tên khoa 
	NGTLAP smalldatetime,--Ngày thành lập khoa 
	TRGKHOA char(4), --Trưởng khoa (mã giáo viên) 
)

CREATE TABLE MONHOC (
MAMH varchar(10) PRIMARY KEY, --Mã môn học (khóa chính) 
TENMH  varchar(40),--Tên môn học
TCLT tinyint,--Số tín chỉ lý thuyết 
TCTH  tinyint,--Số tín chỉ thực hành
MAKHOA varchar(4) REFERENCES KHOA(MAKHOA),--Môn học thuộc khoa nào 
)

CREATE TABLE DIEUKIEN (
	MAMH  varchar(10) REFERENCES MONHOC(MAMH),--Mã môn học (thuộc tính khóa)
	MAMH_TRUOC  varchar(10) REFERENCES MONHOC(MAMH),--Mã môn học phải học trước (thuộc tính khóa)
	PRIMARY KEY (MAMH,MAMH_TRUOC)
)

CREATE TABLE GIAOVIEN (
	MAGV char(4) PRIMARY KEY,--Mã giáo viên (khóa chính) 
	HOTEN varchar(40),--Họ tên 
	HOCVI varchar(10), --  Học vị
	HOCHAM varchar(10),--học hàm 
	GIOITINH  varchar(3),--Giới tính
	NGSINH  smalldatetime, --Ngày sinh
	NGVL smalldatetime,--ngày vào làm việc
	HESO  numeric(4,2),--Hệ số lương
	MUCLUONG  money,--Mức lương
	MAKHOA varchar(4) REFERENCES KHOA(MAKHOA),--Thuộc khoa nào 
)

CREATE TABLE LOP (
	MALOP char(3) PRIMARY KEY,--Mã lớp (khóa chính) 
	TENLOP varchar(40),--Tên lớp 
	TRGLOP  char(5),--Lớp trưởng (mã học viên)
	SISO tinyint,--Sỉ số lớp 
	MAGVCN char(4),--Mã giáo viên chủ nhiệm 
)


CREATE TABLE HOCVIEN (
	MAHV char(5) PRIMARY KEY,--Mã học viên (khóa chính) 
	HO varchar(40),--Họ và tên lót 
	TEN varchar(10),--Tên 
	NGSINH smalldatetime,--Ngày sinh 
	GIOITINH varchar(3),--Giới tính 
	NOISINH varchar(40),--Nơi sinh 
	MALOP char(3) REFERENCES LOP(MALOP),--Mã lớp 
)

CREATE TABLE GIANGDAY(
	MALOP char(3) REFERENCES LOP(MALOP),--Mã lớp (thuộc tính khóa) 
	MAMH varchar(10) REFERENCES MONHOC(MAMH),--Mã môn học (thuộc tính khóa) 
	MAGV char(4) REFERENCES GIAOVIEN(MAGV),--Mã giáo viên 
	HOCKY tinyint,--Học kỳ 
	NAM smallint,--Năm học 
	TUNGAY smalldatetime,--Ngày bắt đầu môn học
	DENNGAY smalldatetime,--ngày kết thúc môn học
	PRIMARY KEY (MALOP,MAMH)
)

CREATE TABLE KETQUATHI (
	MAHV  char(5) REFERENCES HOCVIEN(MAHV),--Mã học viên (thuộc tính khóa)
	MAMH  varchar(10) REFERENCES MONHOC(MAMH),--Mã môn học (thuộc tính khóa)
	LANTHI  tinyint,--Lần thi (thuộc tính khóa)
	NGTHI smalldatetime,--Ngày thi 
	DIEM numeric(4,2),--Điểm thi 
	KQUA varchar(10)--Kết quả thi 
